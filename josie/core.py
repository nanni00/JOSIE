...
def JOSIEAlgorithm(
    ...
    for i in range(len(freqs)):
        if i == 0:
            ...
            read_list_costs[i] = read_list_costs[i - 1] + db.read_list_cost(
                freqs[i] + 1
            )
    ...
    for i in tqdm(
        ...
        for entry in tqdm(
            ...
            if kth_overlap(h, k) >= max_overlap_unseen_candidate:
                continue
            counter[entry.set_id] = CandidateEntry(
                entry.set_id, entry.size, entry.match_position, i, skipped_overlap
            )
        ...
        for candidate in tqdm(
            ...
            if len(h) >= k:
                ...
                if (
                    not fast_estimate
                    and num_candidate_expensive * len(candidates)
                ...
                if not fast_estimate:
                    merge_lists_benefit = process_candidates_update(
                        db, kth, candidates, counter, ignores
                    )
                ...
                probe_set_benefit = read_set_benefit(
                    query_size,
                    kth,
                    kth_overlap_after_push(h, k, candidate.estimated_overlap),
                    candidates,
                    read_list_costs,
                    fast_estimate,
                )
                ...
                if (
                    probe_set_benefit - probe_set_cost
                    < merge_lists_benefit - merge_lists_cost
                ):
                    break
            ...
            if (
                fast_estimate
                or (num_candidate_expensive + 1) * len(candidates)
                > expensive_estimation_budget
            ):
                merge_lists_benefit -= read_lists_benefit_for_candidate(
                    db, candidate, fast_estimate_kth_overlap
                )
            ...
            if candidate.suffix_length() > 0:
                s = db.get_set_tokens_by_suffix(
                    candidate.id, candidate.latest_match_position + 1
                )
