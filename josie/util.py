...
class CandidateEntry:
    def __init__(
        ...
        self.partial_overlap = (
            skipped_overlap + 1
        )  # overlapping token skipped + the token at the current position
    ...
    def est_overlap(self, query_size, query_current_position):
        self.estimated_overlap = int(
            (
                self.partial_overlap
                / (query_current_position + 1 - self.query_first_match_position)
            )
            * (query_size - self.query_first_match_position)
        )
    ...
    def est_cost(self, db: DBHandler):
    ...
    def est_truncation(self, query_size, query_current_position, query_next_position):
        self.estimated_next_truncation = int(
            (
                (query_next_position - query_current_position)
                / (query_size - self.query_first_match_position)
            )
            * (self.size - self.first_match_position)
        )
        return self.estimated_next_truncation

    def est_next_overlap_upperbound(
        self, query_size, query_current_position, query_next_position
    ):
        ...
        additional_overlap = int(
            (self.partial_overlap / query_prefix_length) * query_jump_length
        )
        next_latest_matching_position = (
            int(
                (query_jump_length / (query_size - self.query_first_match_position))
                * (self.size - self.first_match_position)
            )
        ...
        self.estimated_next_upperbound = (
            ...
            + min(
                ...
                self.size - next_latest_matching_position - 1,
            )
        )
    ...
    def check_min_sample_size(self, query_current_position, batch_size):
        return (
            query_current_position - self.query_first_match_position + 1
        ) > batch_size
...
class ByFutureMaxOverlap:
    ...
    def __lt__(self, other):
        return (
            self.candidates.estimated_next_upperbound
            < other.candidates.estimated_next_upperbound
        )


def upperbound_overlap_unknown_candidate(
    query_size, query_current_position, prefix_overlap
):
...
def read_lists_benefit_for_candidate(db: DBHandler, ce: CandidateEntry, kth_overlap):
    if kth_overlap >= ce.estimated_next_upperbound:
        return ce.estimated_cost
    return ce.estimated_cost - db.read_set_cost(
        ce.suffix_length() - ce.estimated_next_truncation
    )


def process_candidates_init(
    db: DBHandler,
    query_size,
    query_current_position,
    ...
    ignores,
) -> tuple[float | Any, int, list[CandidateEntry]]:
    ...
    for ce in list(candidates.values()):
        ...
        ce.est_next_overlap_upperbound(
            query_size, query_current_position, next_batch_end_index
        )
...
def read_set_benefit(
    query_size, kth_overlap, kth_overlap_after_push, candidates, read_list_costs, fast
):
    b = 0.0
